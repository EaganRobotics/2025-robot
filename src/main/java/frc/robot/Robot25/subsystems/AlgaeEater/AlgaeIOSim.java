...
public class AlgaeIOSim implements AlgaeIO {
  ...
  private static final Transform3d ALGAE_LOADED_POSE = new Transform3d(Inches.of(12.49), Inches.zero(),
      Inches.of(28.278), Rotation3d.kZero);
  private static final Transform3d MANIPULATOR_POSE = new Transform3d(Inches.of(15.042), Inches.zero(),
      Inches.of(22.987), Rotation3d.kZero);
  ...
  public void updateInputs(AlgaeIOInputs inputs) {
    ...
    if (!hasAlgae) {
      if (algaeIntakeAppliedVolts.lt(Volts.of(-2.0))) {
        var algaePoseIndex = grabbableAlgaeIndex(manipulatorPose);
        if (algaePoseIndex.isPresent()) {
          algaeIndex = algaePoseIndex.get();
          hasAlgae = true;
          algaeLoadTime = Timer.getFPGATimestamp();
          grabbedAlgaePose = RobotContainer.simAlgaePoses[algaeIndex];
          RobotContainer.simAlgaePoses[algaeIndex] = SimConstants.QUEENED_GAMEPIECE_POSE;
        }
      }
    } else if (t < 1) {
      if (grabbedAlgaePose.minus(manipulatorPose).getTranslation().getNorm() > MAX_ALGAE_DISTANCE.in(Meters)
          || algaeIntakeAppliedVolts.gt(Volts.of(-2.0))) {
        hasAlgae = false;
        RobotContainer.simAlgaePoses[0] = SimConstants.QUEENED_GAMEPIECE_POSE;
        RobotContainer.simAlgaePoses[algaeIndex] = grabbedAlgaePose;
      } else {
      ...
      if (algaeIntakeAppliedVolts.gt(Volts.of(2.0))) {
        ...
        var shootHeight = Elevator.getInstance().getCurrentHeight().plus(MANIPULATOR_POSE.getMeasureZ())
            .minus(ElevatorConstants.MIN_HEIGHT);
        var angle = Degrees.of(5);
        var projectile = new ReefscapeAlgaeOnFly(driveSim.getSimulatedDriveTrainPose().getTranslation(),
            ...
            MetersPerSecond.of(algaeIntakeAppliedVolts.in(Volts) / 3), angle);
